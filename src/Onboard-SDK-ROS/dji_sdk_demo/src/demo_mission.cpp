...
WayPointSettings end_wp;
bool reached_first;

bool AreSame(double a, double b)
{
    return fabs(a - b) < (0.000001 / C_PI * 180)/6;
}
...
{
  gps_pos = *msg;
  ROS_INFO("Current_gps: %f \t%f \n", gps_pos.latitude, gps_pos.longitude);
  ROS_INFO("END_wp : %f \t%f \n", end_wp.latitude, end_wp.longitude);
  if (AreSame(gps_pos.latitude, end_wp.latitude) && AreSame(gps_pos.longitude, end_wp.longitude)){
    land();
  }
}
